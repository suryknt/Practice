...
//			System.out.println("i:: "+i+" pow2:: "+pow2[i]);
		...
		for(int i=1;i!=0; i<<=1) {
			...
			for(int j=0;j<n;j++) {
				...
				if((a[j] & i)==0)clearBitCount++;
			}
			System.out.println("i:: "+i+" clearBitCount:: "+clearBitCount+" pow2[clearBitCount]:: "+pow2[clearBitCount]);
			long subsetSum=pow2[n]-pow2[clearBitCount];
			System.out.println("pow2[n]::"+pow2[n]+" subsetSum:: "+subsetSum);
//			subSetSum-=1;
			sum += i*subsetSum;
			sum%=mod;
		}
